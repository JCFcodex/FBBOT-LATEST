...
module.exports.config = {
  ...
  usePrefix: true,
  usage:
    "{pref}animirror [modelNumber]\nexample: {pref}animirror 2\n\nHere are the available models:\n" +
  ...
  try {
    ...
    if (!modelNumber || isNaN(modelNumber) || !models[modelNumber]) {
      return api.sendMessage(
        {
          body:
            "Invalid model number. Please provide a valid model number from the list.\n\n type animirror list to view all list ",
        },
        event.messageID,
        event.threadID
      );
    ...
    const response = await axios.get(
      `https://simoapi-aimirror.onrender.com/generate?imageUrl=${encodedImageUrl}&modelNumber=${modelNumber}`
    );

    const generatedImageUrl = response.data.imageUrl;

    const { data: imageBuffer } = await axios.get(generatedImageUrl, {
