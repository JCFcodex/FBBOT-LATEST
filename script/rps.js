...
module.exports.run = async ({ api, event, args, Currencies }) => {
  ...
  if (!args[0] || !choices.includes(args[0])) {
    api.sendMessage(
      "Invalid choice. Please select one of the following: [rock/paper/scissors] [bet]. Please try again.",
      threadID,
      messageID
    );
    return;
  ...
  if (!args[1] || isNaN(args[1]) || parseInt(args[1]) <= 0) {
    api.sendMessage(
      "Invalid amount. Please specify a valid amount to bet.",
      threadID,
      messageID
    );
    return;
  ...
  if (userMoney < args[1]) {
    return api.sendMessage(
      `Your balance is not enough to place this bet`,
      threadID,
      messageID
    );
  }
  ...
  api.sendMessage(
    `You chose ${
      args[0]
    } and the bot chose ${selectedChoice}. ${getOutcomeMessage(
      result,
      amount,
      winnings
